...
Şema artık Alembic (Flask-Migrate) ile yönetiliyor. Start command otomatik halleder:
```
flask db upgrade && flask seed && gunicorn ...
```

**Modelde değişiklik yaptığında (lokalde):**
```bash
export FLASK_APP=app.py
flask db migrate -m "yeni kolon: user.xyz"   # script üretir → migrations/versions/
flask db upgrade                              # lokalde uygula ve test et
git add migrations/ && git commit && git push # Render deploy'da otomatik uygulanır
```

**⚠️ Mevcut (tabloları zaten oluşmuş) bir veritabanına ilk kez geçiyorsan:**
`flask db upgrade` "table already exists" hatası verir. Bir kerelik şunu çalıştır:
```bash
flask db stamp head   # "şema zaten güncel" işareti koyar, tablo oluşturmaz
```
